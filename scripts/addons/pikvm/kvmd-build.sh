...
package_kvmd() {
	...
	mkdir -p "$pkgdir/etc/kvmd/scripts"
	chmod 755 "$pkgdir/etc/kvmd/scripts"
	install -Dm755 -t "$pkgdir/etc/kvmd/scripts" "contrib/nanokvm/scripts"/*.sh

	mkdir -p "$pkgdir/usr/bin"
	chmod 755 "$pkgdir/usr/bin"
	install -Dm755 -t "$pkgdir/usr/bin" "contrib/nanokvm/bin"/vcgencmd
...
for _variant in "${_variants[@]}"; do
	...
	_base=${_platform%-*}
	_video=${_platform#*-}
	eval "package_kvmd_platform_${_platform/-/_}_${_board}() {
		#cd \"kvmd-\$pkgver\"
...
package_kvmd_platform_v2_hdmiusb_rpi4
